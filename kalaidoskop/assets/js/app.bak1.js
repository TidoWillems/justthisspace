...
// Intro nach DOM-Start weich zeigen
requestAnimationFrame(() => {
  ...
  if ('serviceWorker' in navigator) {
    ...
    const dir = location.pathname.endsWith('/') ? location.pathname
  ...
  fontDec?.addEventListener('click', ()=>{ const s=clamp(getSize()-1,12,22); setSize(s); localStorage.setItem('kalanf-font', s); setScrollOffset(); });
  fontInc?.addEventListener('click', ()=>{ const s=clamp(getSize()+1,12,22); setSize(s); localStorage.setItem('kalanf-font', s); setScrollOffset(); });

addEventListener('load', setScrollOffset);
...
// wenn die PWA-Install-Schaltfläche sichtbar wird (kann die Höhe ändern)
window.addEventListener('beforeinstallprompt', ()=> { setTimeout(setScrollOffset, 0); }, { once:true });

  // --- „Man sagt:“ markieren + per-Section Controls ---
  const sections = Array.from(document.querySelectorAll('main section')).filter(s=>!s.classList.contains('intro'));
  sections.forEach((sec)=>{
    ...
    const btnMS=document.createElement('button'); btnMS.type='button'; btnMS.textContent='📜'; btnMS.title='„Man sagt:“ in diesem Abschnitt ein/aus'; btnMS.setAttribute('aria-label','„Man sagt:“ in diesem Abschnitt ein/aus');
    btnMS.addEventListener('click', ()=>{ if (man) man.hidden = !man.hidden; });
    const btnTOC=document.createElement('button'); btnTOC.type='button'; btnTOC.textContent='⤴︎'; btnTOC.title='Zum Inhaltsverzeichnis'; btnTOC.setAttribute('aria-label','Zum Inhaltsverzeichnis');
    btnTOC.addEventListener('click', ()=>document.getElementById('tocWrap').scrollIntoView({behavior:'smooth',block:'start'}));
    const btnShare=document.createElement('button'); btnShare.type='button'; btnShare.textContent='🔗'; btnShare.title='Link zu diesem Abschnitt'; btnShare.setAttribute('aria-label','Link zu diesem Abschnitt');
    ...
    ctrl.append(btnMS,btnTOC,btnShare); sec.appendChild(ctrl);
  });

// Scroll-Offset an die Höhe der Top-Controls anpassen
...
// nach setScrollOffset(); und dem resize-Listener:
addEventListener('orientationchange', setScrollOffset);
  ...
  toggleAll?.addEventListener('click', ()=>{
    ...
    localStorage.setItem('kalanf-hide-mansagt', body.classList.contains('hide-mansagt') ? '1':'0');
  });

  // --- Zufalls-Sprung ---
  document.getElementById('randomJump')?.addEventListener('click', ()=>{
    const sec = sections[Math.floor(Math.random()*sections.length)];
    if (!sec) return; sec.scrollIntoView({behavior:'smooth',block:'start'}); sec.classList.add('reveal');
  });
  ...
  document.getElementById('downloadBtn')?.addEventListener('click', ()=>{
    ...
    a.href=url; a.download=`kalanf-${ts}.html`; document.body.appendChild(a); a.click(); a.remove();
    setTimeout(()=>URL.revokeObjectURL(url),2000);
  });

  // --- Reveal on scroll ---
  if ('IntersectionObserver' in window) {
    const io=new IntersectionObserver((ents)=>ents.forEach(e=>{if(e.isIntersecting)e.target.classList.add('reveal')}),{threshold:.08});
  ...
  const slugify=(s)=>s.toLowerCase().replace(/[^\p{L}\p{N}\s-]/gu,'').replace(/\s+/g,'-').replace(/-+/g,'-').slice(0,120);
  ...
  heads.forEach((h)=>{
    ...
    pl.href='#'+id; pl.className='permalink'; pl.title='Link zu diesem Abschnitt kopieren'; pl.textContent='¶';
  ...
  const renderTOC = ()=>{
    ...
    items.forEach(({id,title})=>{
      ...
      const copy=document.createElement('button'); copy.type='button'; copy.className='copylink'; copy.textContent='⎘'; copy.title='Link kopieren'; copy.setAttribute('aria-label','Link kopieren');
      copy.addEventListener('click', async (ev)=>{ ev.preventDefault(); const url=location.origin+location.pathname+'#'+id; try{ await navigator.clipboard.writeText(url); }catch(e){} });
  ...
  // A–Z Index (aktiv/dim je nach vorhandenen Initialen)
  if (lettersWrap) {
  ...
  // Aktives TOC-Item hervorheben beim Scrollen
  const idToLi = ()=> {
    const map=new Map();
  ...
  // TOC-Highlight bei direktem Hash-Aufruf / Hash-Wechsel
