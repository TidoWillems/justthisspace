(()=> {
  ...
  // --- Service Worker (mit optionalem ?refresh=xyz)
...
navigator.serviceWorker.register(swUrl, { scope: dir }).catch(()=>{});
  ...
  const applyTheme = (mode, persist=true) => {
    if (mode === 'light') {
      root.classList.add('light','light-set'); root.classList.remove('dark-set');
      icon.textContent = '🌙';
    } else {
      root.classList.remove('light'); root.classList.add('dark-set');
      icon.textContent = '☀️';
    }
  ...
  const getSize = ()=>parseFloat(getComputedStyle(root).getPropertyValue('--base-size'));
  ...
  fontDec?.addEventListener('click', ()=>{ const s=clamp(getSize()-1,12,22); setSize(s); localStorage.setItem('kalanf-font', s); });
  fontInc?.addEventListener('click', ()=>{ const s=clamp(getSize()+1,12,22); setSize(s); localStorage.setItem('kalanf-font', s); });
  ...
  sections.forEach((sec)=>{
    ...
    const btnMS=document.createElement('button'); btnMS.type='button'; btnMS.textContent='📜'; btnMS.title='„Man sagt:“ in diesem Abschnitt ein/aus';
    btnMS.addEventListener('click', ()=>{ if (man) man.hidden = !man.hidden; });
    const btnTOC=document.createElement('button'); btnTOC.type='button'; btnTOC.textContent='⤴︎'; btnTOC.title='Zum Inhaltsverzeichnis';
    btnTOC.addEventListener('click', ()=>document.getElementById('tocWrap').scrollIntoView({behavior:'smooth',block:'start'}));
    const btnShare=document.createElement('button'); btnShare.type='button'; btnShare.textContent='🔗'; btnShare.title='Link zu diesem Abschnitt';
  ...
  const renderTOC = ()=>{
    ...
    items.forEach(({id,title})=>{
      ...
      const copy=document.createElement('button'); copy.type='button'; copy.className='copylink'; copy.textContent='⎘'; copy.title='Link kopieren';
